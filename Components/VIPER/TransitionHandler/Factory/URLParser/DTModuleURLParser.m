...
@implementation DTModuleURLParserResult
@end

@implementation DTModuleURLParser
...
+ (DTModuleURLParserResult *)parseURL:(NSURL *)url error:(NSError **)error
{
    if ([self isInAppURL:url]) {
        ...
        DTModuleURLParserResult *result = [DTModuleURLParserResult new];
        ...
        if (error) {
            *error = [NSError errorWithDomain:DTModuleURLParserErrorDomain code:DTModuleURLParserErrorCodeBadScheme
                                     userInfo:@{
...
{
    return [NSString stringWithFormat:@"DT%@ViewController", moduleName];
}


+ (DTModuleURLParserResult *)parseInAppURL:(NSURL *)url error:(NSError **)error
{
    DTModuleURLParserResult *result = [DTModuleURLParserResult new];
    ...
    if ([components count] > 0) {
        ...
        if (error) {
            *error = [NSError errorWithDomain:DTModuleURLParserErrorDomain code:DTModuleURLParserErrorCodeBadUrl
                                    userInfo:@{
