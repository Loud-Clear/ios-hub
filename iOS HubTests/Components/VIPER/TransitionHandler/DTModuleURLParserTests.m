...
@interface DTModuleURLParserTests : XCTestCase

@end

@implementation DTModuleURLParserTests

- (void)test_with_incorrect_scheme
{
    NSURL *url = [NSURL URLWithString:@"ftp://123123"];
    NSError *error = nil;
    [DTModuleURLParser parseURL:url error:&error];
    XCTAssertNotNil(error);
    XCTAssertEqual(error.code, DTModuleURLParserErrorCodeBadScheme);

    NSURL *url2 = [NSURL URLWithString:@"file:///123123"];
    [DTModuleURLParser parseURL:url2 error:&error];
    XCTAssertNotNil(error);
    XCTAssertEqual(error.code, DTModuleURLParserErrorCodeBadScheme);
}
...
{
    NSString *moduleName = [DTModuleURLParser moduleNameFromViewControllerClassName:@"DTSomeModuleViewController"];
    XCTAssertEqualObjects(moduleName, @"SomeModule");
}

- (void)test_controller_name_parsing_from_module_name
{
    NSString *controllerName = [DTModuleURLParser viewControllerClassNameFromModuleName:@"SomeModule"];
    XCTAssertEqualObjects(controllerName, @"DTSomeModuleViewController");
}
